...
module tasmin_eq #(parameter N=2, parameter width = 16)(
input logic clk,
input logic rst_n,
input  [7:0]  ui_in,
output [7:0]  uo_out,
...
//filtercoefficients
logic signed [width-1:0] bL [0:N] = '{16'sd7, 16'sd15, 16'sd7};
logic signed [width-1:0] aL [1:N] = '{-16'sd31776, 16'sd15421};
logic signed [width-1:0] bM [0:N] = '{16'sd7429, 16'sd0, -16'sd7429}; 
logic signed [width-1:0] aM [1:N] = '{-16'sd17256, 16'sd1525};
logic signed [width-1:0] bH [0:N] = '{16'sd5505, -16'sd11010, 16'sd5505};
logic signed [width-1:0] aH [1:N] = '{-16'sd2743, 16'sd2892};
	...
	if (!rst_n) begin
	   ...
	   if (~(&data_write_n)) begin
	       case (address)
               ...
               default: begin
                    // Do nothing for unrecognized addresses
               end          
